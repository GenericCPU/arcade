...
// never leaks into it.
//
// The verdict peer answers 400, the status Teranode assigns to the policy
// family. It used to answer 422 here, which was a mislabel: 422 is reachable
// from exactly one branch of httpStatusForTxError — ErrTxMissingParent — so a
// 422 carrying the opaque PROCESSING catch-all is now read as the
// missing-parent CONDITION rather than a verdict, and correctly writes no
// terminal row at all. That routing is pinned by row D2 of
