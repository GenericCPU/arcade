...
// lineIsOpaqueProcessing reports whether a line is Teranode's PROCESSING
// catch-all — the code its public error boundary falls back to when no cause
// in the chain is on the client-safe allowlist. Such a line says only "this
// transaction did not make it": the actual cause (missing parent, frozen utxo,
// coinbase immature, a BDK CGO exception, a storage failure inside the
// validator) was stripped at the boundary. It is therefore the one line shape
// where the peer's HTTP status carries strictly more information than its
// message, and the only one for which arcade consults that status.
